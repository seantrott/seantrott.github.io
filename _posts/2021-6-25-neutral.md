...
So what exactly are the challenges to this theory?

## Neutral Theory in Molecular Evolution

As I hope I've made clear, I'm not a molecular biologist, so I hesitate to cover this debate at all. At the same time, given that neutral theory originated in the study of molecular evolution, I'd be remiss if I didn't mention it in a review of neutral theory. So all that being said: take what I write in this section with more than a few grains of salt.

First, recall that the original context for Kimura's (1968) neutral theory was an intellectual paradigm in which selection was assumed as *the* major driver of changes in allele frequency over time. He was pushing back against this, arguing instead that a large number of evolutionary changes are "caused by the 'random drift' of selectively equivalent mutant genes'" (Kimura, 1979, pg. 98). An important feature of this model is that mutations (and subsequent changes in allele fixation) occur at a relatively constant rate (a "molecular clock"); that is, these changes are driven by some underlying biological mechanism that's indifferent to relative fitness, as long as the changes aren't outright deleterious.

According to Hey (1999), this view sparked a contentious, decades-long debate between champions of **selectionist** and **neutralist** explanations. This debate seems to continue to today (see Kern & Hanh, 2018 vs. Jensen et al, 2019 for two opposing perspectives), though I'm not sure to what extent one view has reached field-wide consensus; as far as I can tell, opponents of the "strongest" version of neutral theory still seem to acknowledge that it's played a useful role as a statistical baseline against which to measure claims about adaptation (Hey, 1999; Kern & Hanh, 2018).

As far as I can tell, 


(****TODO)
(Discuss main critiques)
...One early critique of the neutral model was that its predictions were indistiguishable from a "balancing selection" model (Kreitman, 1996). 


## Neutral Theory and Cultural Evolution

What about neutral models of cultural change, like the baby name example we saw early?
...
In other words, don't just assume it's a question of drift vs. selection. Instead, include each of these factors as parameters in a model, and treat it as an exercise in parameter estimation. What's the relative magnitude of the drift parameter vs. the selection parameter?
