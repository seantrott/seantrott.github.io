...
[Piantadosi et al (2013)](https://arxiv.org/pdf/1307.6726.pdf) make a similar argument: 
...
Either way, my view is that building out a neutral model––building a baseline––should be an important part of making any evolutionary claim, if only as a kind of disciplining exercise ([^3]). [Just-so stories](https://en.wikipedia.org/wiki/Just-so_story) are satisfying to construct (and to read), and that makes them all the more tempting. Building a baseline makes our task a little harder, but I think that's a good thing.
